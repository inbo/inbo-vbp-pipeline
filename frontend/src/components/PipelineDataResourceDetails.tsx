...
function EmrErrorDetails({ cause }: { cause: EmrErrorCause }) {
  const logUrl = `${settings.logsUrl}?refresh=1m&var-log_stream=${cause.Step.Id}&from=${cause.Step.Status.Timeline.StartDateTime}&to=${cause.Step.Status.Timeline.EndDateTime + 5 * 60 * 1000}`;
  return (
    <>
...
function BatchErrorDetails({ cause }: { cause: BatchErrorCause }) {
  const logUrl = `${settings.logsUrl}?refresh=1m&var-log_stream=${cause.Container.LogStreamName.replaceAll(
    "/",
    "\\/",
