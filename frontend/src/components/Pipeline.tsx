...
export const Pipeline = ({
  ...
  const [
    cancelPipeline,
    { loading: cancelPipelineLoading, error: cancelPipelineError },
  ] = useMutation(CANCEL_PIPELINE);
  ...
  const [dataResourceFilter, setDataResourceFilter] =
    useState<DataResourceFilter>({});
  ...
  const pipelineInput = useMemo(() => {
    try {
      return JSON.parse(data?.pipeline?.input || "{}") as StartPipelineInput;
    } catch (e) {
    ...
    return (
      stats && (
        <PipelineProgress
          ...
          bulkAll={(pipelineInput as any).dataResources === "all"}
        />
      )
    );
  }, [pipelineId, data, setDataResourceFilter]);

  const dataResourcesCount = stats?.steps.find(
    (step) => step.step === dataResourceFilter.step,
  )?.[
      ...
      try {
        ...
        if (onlyFailed) {
          ...
          cause
            ...
            .map(({ dataResourceId }) => dataResourceId)
            .forEach((id) => params.append("dr", id));
        } else {
  ...
  return (
    <div className="pipeline">
      <div className="pipeline-details">
        {showHeader && (
          <div className="pipeline-header-container">
            ...
            {data.pipeline.status === PipelineStatus.Running ? (
              ...
              <Button onClick={() => copyPipeline(false)} disabled={loading}>
                <ContentCopyIcon />
              </Button>
            )}
            {data.pipeline.status === PipelineStatus.Failed &&
              data.pipeline.error === "SomeDataResourceProcessingFailed" && (
                <Button onClick={() => copyPipeline(true)} disabled={loading}>
                  <DifferenceIcon />
                </Button>
              )}
          </div>
        )}
        <div className="pipeline-progress">{progress}</div>
        ...
        {showDetails && (
          <div>
            Input: <pre>{JSON.stringify(pipelineInput!, null, 2)}</pre>
            {data?.pipeline?.status == PipelineStatus.Succeeded && (
              <div>
                Output:{" "}
                <pre>
                  {JSON.stringify(
                    JSON.parse(data?.pipeline?.output!)!,
                    null,
                    2,
                  )}
                </pre>
            ...
            {data.pipeline?.stoppedAt && (
              <div>Stopped At: {data?.pipeline?.stoppedAt}</div>
            )}
          </div>
        )}
      </div>
      {dataResourceFilter.step !== undefined && (
        <div className="pipeline-step-data-resources">{dataResources}</div>
      )}
    </div>
  );
};
